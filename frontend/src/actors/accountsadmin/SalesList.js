...
const SalesList= ({ userRole }) => {
  ...
  const fetchData = (sales_jouranl_id) => {
    //  alert(role_id)
    ...
    axios.get('/api/salesJouranl-list', config)
      .then(response => {
  ...
  const columns = useMemo(() => [
    { Header: '#', accessor: 'sales_jouranl_id' },
    ...
    { Header: 'Date', accessor: 'date' },
    {
      ...
      Cell: ({ row }) => (
        <>
          ...
          <Link to={`/UpdateSales/${row.original.sales_jouranl_id}`}
            style={{ marginRight: '10px', color: "blue" }}>
            <FontAwesomeIcon icon={faPencil} />
          </Link>

          <FontAwesomeIcon icon={faTrash} onClick={() => handleDelete(row.original.sales_jouranl_id)}
  ...
  const handleDelete = (sales_jouranl_id) => {
    const token = localStorage.getItem('token');
    const config = {
      headers: {
        Authorization: `Bearer ${token}`,
    ...
    axios.delete(`/api/delete-salesJouranl/${sales_jouranl_id}`, config)
      .then(response => {
        setData(prevData => prevData.filter(user => user.sales_jouranl_id !== sales_jouranl_id));
        // Show success toast
  ...
  const handleUpdateUser = (sales_jouranl_id) => {
    navigate(`/UpdateSales/${sales_jouranl_id}`);
  };
          ...
          <div className="col-xs-12">
            <div className="box">
              <div className="box-header">
                <h3 className="box-title pull-left"><b>Sales List</b></h3>
                <div >
                  <input
                    type="text"
                    value={globalFilter || ''}
