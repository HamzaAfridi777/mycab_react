...
const RoleList = ({ userRole }) => {
  ...
  const fetchData = (role_id) => {
    ...
    axios.get('/api/users-role-list', config)
      .then(response => {
    ...
    
    {
      ...
      Cell: ({ row }) => (
        <>
          ...
          <Link to={`/UpdateRole/${row.original.role_id}`}
            style={{ marginRight: '10px', color: "blue" }}>
            <FontAwesomeIcon icon={faPencil} />
          </Link>

          <FontAwesomeIcon icon={faTrash} onClick={() => handleDelete(row.original.role_id)}
  ...
  const handleDelete = (role_id) => {
    ...
    axios.delete(`/api/delete-role/${role_id}`, config)
      .then(response => {
        // Remove the deleted user from the state
        setData(prevData => prevData.filter(role => role.role_id !== role_id));
        // Show success toast
        toast.success('Role deleted successfully', { position: toast.POSITION.TOP_CENTER, autoClose: 2000 });
          ...
          <div className="col-xs-12">
            <div className="box">
              <div className="box-header">
                <h3 className="box-title pull-left"><b>Roles List </b></h3>
                <div >
                  <input
                    type="text"
                    value={globalFilter || ''}
