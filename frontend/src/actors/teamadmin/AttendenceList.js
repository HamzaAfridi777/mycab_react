...
const AttendenceList = ({ userRole }) => {
  ...
  const fetchData = (attendance_id) => {
    const config = {
    };
    axios.get('/api/attendance-list', config)
      .then(response => {
 ...
 
    {
      ...
      Cell: ({ row }) => (
        <>
          ...
          <Link to={`/UpdateAttendence/${row.original.attendance_id}`}
            style={{ marginRight: '10px', color: "blue" }}>
            <FontAwesomeIcon icon={faPencil} />
          </Link>

          <FontAwesomeIcon icon={faTrash} onClick={() => handleDelete(row.original.attendance_id)}
  ...
  const handleDelete = (attendance_id) => {
    ...
    axios.delete(`/api/delete-attendance/${attendance_id}`, config)
      .then(response => {
        setData(prevData => prevData.filter(user => user.attendance_id !== attendance_id));
        // Show success toast
        toast.success('Attendance deleted successfully', { position: toast.POSITION.TOP_CENTER, autoClose: 2000 });
  ...
  const handleUpdateUser = (attendance_id) => {
    navigate(`/UpdateAttendence/${attendance_id}`);
  };
          ...
          <div className="col-xs-12">
            <div className="box">
              <div className="box-header">
                <h3 className="box-title pull-left"><b>Attendence List</b></h3>
                <div >
                  <input
                    type="text"
                    value={globalFilter || ''}
