...
const FleetList = ({ userRole }) => {
   ...
   
    {
      ...
      Cell: ({ row }) => (
        <>
          ...
          <Link to={`/UpdateFleet/${row.original.fleet_id}`}
            style={{ marginRight: '10px', color: "blue" }}>
            <FontAwesomeIcon icon={faPencil} />
          </Link>

          <FontAwesomeIcon icon={faTrash} onClick={() => handleDelete(row.original.fleet_id)}
  ...
  const handleDelete = (fleet_id) => {
    ...
    axios.delete(`api/delete-fleet/${fleet_id}`, config)
      .then(response => {
        // Remove the deleted user from the state
        setData(prevData => prevData.filter(fleet => fleet.fleet_id !== fleet_id));
        // Show success toast
        toast.success('Fleet deleted successfully', { position: toast.POSITION.TOP_CENTER, autoClose: 2000 });

      })
      .catch(error => {
        console.error('Error deleting user:', error);
        // Show error toast if deletion fails
        toast.error('Failed to delete Fleet', { position: toast.POSITION.TOP_CENTER, autoClose: 2000 });
      });
  };
  const handleUpdateUser = (fleet_id) => {
    navigate(`/UpdateFleet/${fleet_id}`);
          ...
          <div className="col-xs-12">
            <div className="box">
              <div className="box-header">
                <h3 className="box-title pull-left"><b>Fleet List</b></h3>
                <div >
                  <input
                    type="text"
                    value={globalFilter || ''}
