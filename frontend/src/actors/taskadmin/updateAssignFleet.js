...
const fetchAssignFleet = async () => {
    try {
        const response = await axios.get(`api/update-assign-fleet/${assign_fleet_id}`);
        return response;
    } catch (error) {
        throw error;
    }
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const [driversResponse, fleetsResponse, assignFleetResponse] = await Promise.all([fetchDrivers(), fetchFleets(), fetchAssignFleet()]);
